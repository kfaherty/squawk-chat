...
class Chat extends Component {
  ...
  handleChange(event) {
    ...
    if (this.props.chat.type === 3) {
      // this only needs to run if this is a private chat.
  ...
  render() {
    ...
    return (
      <div className="chat-window">
        ...
        <div
          className={"chat-wrap " + (this.props.selectedChat ? "" : "hidden")}
        >
          <div
            className={
              "chat-contain " + (this.props.userListOpen ? "" : "full")
            }
          >
                    ...
                    case 0:
                      return (
                        <div className="chat-subtitle">
                          <ParsedText text={chat.description} />
                        </div>
                      );
                    case 1:
                      return (
            ...
            <div
              className={
                "dropdown " + (this.state.chatMenuOpen ? "visible" : "")
              }
            >
              <div onClick={this.clearSelectedChat} className="list-item">
                <div className="list-icon fi-trash"></div>
                {chat.type === 3 ? "Close Chat" : "Leave Channel"}
              </div>
              {chat.type === 3 && (
                <div onClick={this.openProfile} className="list-item">
                  <div className="list-icon fi-magnifying-glass"></div>Show
                  Profile
                </div>
              )}
              <div
                ...
                className={
                  "list-item " + (this.state.favorited ? "hidden" : "")
                }
              >
                <div className="list-icon fi-star"></div>Favorite
              </div>
              <div
                onClick={this.toggleFavorite}
                className={
              ...
              >
                <div className="list-icon fi-star"></div>Unfavorite
              </div>
              {chat.type === 3 && (
                <div
                  onClick={this.toggleIgnore}
                  className={
                    "list-item " + (this.state.ignored ? "hidden" : "")
                  }
                >
                  <div className="list-icon fi-plus"></div>Ignore
                </div>
              )}
              {chat.type === 3 && (
                <div
                ...
                >
                  <div className="list-icon fi-minus"></div>Unignore
                </div>
              )}
              <div onClick={this.getLogs} className="list-item">
                <div className="list-icon fi-page-filled"></div>Get logs
              </div>
              <div onClick={this.reportSelectedChat} className="list-item">
                <div className="list-icon fi-flag"></div>Report
              </div>
            </div>
            ...
            <div
              className={
                "scroll-button " + (!!this.state.scrolledBottom ? "active" : "")
              }
            ...
            <div className="input-contain">
              <div
                className={"label " + (this.state.inputValue ? "hidden" : "")}
              >
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.selectedChat !== this.props.selectedChat) {
      // scroll to the bottom if you've changed chats
      this.scrollToBottom(true);
      return;
    }

    if (!!this.state.scrolledBottom) {
      // scrolled to bottom already == scroll again.
      this.scrollToBottom();
    }
  }
}
