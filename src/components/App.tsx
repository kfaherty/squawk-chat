...
import {
  ...
  updateStatus,
  setChannelsCallback,
  setJoinedChannelsCallback,
  setFriendsCallback,
  setSelectedChatCallback,
  setSelectedChat,
  setCreateToastCallback,
  setChannelMessagesCallback,
  setChannelUsersCallback,
  setPrivateChannelsCallback
} from "../api/api2";
...
const NotificationTemplate: React.FC<INotificationTemplateProps> = (props) => (
  <div className={"toast-item " + (props.error ? "error" : "")}>
    <div className="content-wrap">
      <div className="toast-header">{props.header}</div>
      <div className="toast-content">
        <ParsedText character={props.character} text={props.text} />
      </div>
    </div>
    <button onClick={props.closeToast}>Dismiss</button>
  </div>
);
  ...
  render() {
    return (
      <div className="app-wrapper">
        ...
        <div
          className={
            "potential-problem " + (this.state.connected ? "" : "visible")
          }
        >
        ...
        <div className={"top-bar " + (this.state.loggedin ? "" : "blurred")}>
          ...
          <nav className="text-buttons-contain">
            <span
              onClick={() => this.setSelectedTab("messages")}
              className={
                "text-button " +
                (this.state.selectedTab === "messages" ? "active" : "")
              }
            >
              Messages
            </span>
            <span
              onClick={() => this.setSelectedTab("channels")}
              className={
                "text-button " +
                (this.state.selectedTab === "channels" ? "active" : "")
              }
            >
              Channels
            </span>
            <span
              onClick={() => this.setSelectedTab("private")}
              className={
                "text-button " +
                (this.state.selectedTab === "private" ? "active" : "")
              }
            >
              Private
            </span>
            <span
              onClick={() => this.setSelectedTab("friends")}
              className={
                "text-button " +
                (this.state.selectedTab === "friends" ? "active" : "")
              }
            >
              Friends
            </span>
            {
              <span
                ...
                className={
                  "text-button " +
                  (this.state.selectedTab === "search" ? "active" : "")
                }
              >
                Search
              </span>
          ...
          >
              ...
              <div className="user-header">
                <div className="user-wrap">
                  ...
                  <div className="user-status">
                    ...
                    {this.state.currentStatusMessage && (
                      <span>
                        :{" "}
                        <ParsedText
                          character={this.state.username}
                          text={this.state.currentStatusMessage}
                        />
                      </span>
                    )}
                  </div>
            ...
            <div
              className={"arrow " + (this.state.userMenuOpen ? "flipped" : "")}
            ></div>
            {this.state.username ? (
          ...
          <div
            className={
              "dropdown " +
              (this.state.userMenuOpen ? "visible " : "") +
              (this.state.userListOpen ? "" : "full")
            }
          >
            <div className="list-item" onClick={() => this.toggleStatus()}>
              <div className="list-icon fi-pencil"></div>Status
            </div>
            <div className="list-item" onClick={() => this.toggleSettings()}>
              <div className="list-icon fi-widget"></div>Settings
            </div>
            <div className="list-item" onClick={() => this.logout()}>
              <div className="list-icon fi-lock"></div>Logout
            </div>
          </div>
          ...
          <div
            className={
              "controls-contain " + (this.state.userListOpen ? "" : "full")
            }
          >
            <div
        ...
        <div
          className={"app-contain " + (this.state.loggedin ? "" : "blurred")}
        >
          {/* MESSAGES */}
          <RoomList
